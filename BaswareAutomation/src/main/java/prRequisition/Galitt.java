...
public class Galitt extends BaseTest {
		...
		for (int i = 1; i <=excelData.size() ; i++) {
			...
			try {
				...
				EditCoding editCoding = new EditCoding(driver);
					editCoding.viewDetailsGalitt(javascriptExecutor, excelData.get(i).get("Requestor"), excelData.get(i).get("Delivery Address"), excelData.get(i).get("Nature"), excelData.get(i).get("Agency/Site"),excelData.get(i).get("Type"), excelData.get(i).get("PR tYPE"),excelData.get(i).get("SSP"));
