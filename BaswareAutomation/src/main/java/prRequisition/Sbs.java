		...
		for (int i = 1; i <=excelData.size() ; i++) {
			...
			try {
				...
				Form form = new Form(driver);
					form.editRequisitionSBS(excelData.get(i).get("PRForm"));
				
				test.log(Status.INFO, "Enter Data for Edit Requisition creation ");
				EditRequisition editRequisition = new EditRequisition(driver);
					editRequisition.editCodingSBS(excelData.get(i).get("OrderDesc"), excelData.get(i).get("PurchaseCategory"),excelData.get(i).get("ProductCode"),excelData.get(i).get("Product Name"),excelData.get(i).get("Supplier"), excelData.get(i).get("Quantity"),excelData.get(i).get("UnitPrice"),excelData.get(i).get("Currency"), excelData.get(i).get("PR tYPE"),excelData.get(i).get("PRForm"));
				...
				ViewDetails viewDetails = new ViewDetails(driver);
					viewDetails.getApproveSBS(excelData.get(i).get("Quantity"), excelData.get(i).get("UnitPrice"),excelData.get(i).get("Payment Terms"));
